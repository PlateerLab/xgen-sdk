...
class BaseModel(ABC):
    ...
    def get_indexes(self) -> List[tuple]:
        ...
        Returns:
            ``[("인덱스명", "컬럼1, 컬럼2 DESC"), ...]`` — 일반 인덱스.

            **유일성**이 필요하면 세 번째 원소로 ``True`` 를 준다::

                ("uq_deploy_meta_workflow", "workflow_id", True)

            유일 인덱스는 성능 장치가 아니라 **불변의 집행**이다. 애플리케이션
            코드만 지키는 규칙은 다른 경로(새 코드·배치·수동 INSERT)가 조용히
            깨뜨릴 수 있다. DB 가 거절해야 규칙이 된다.

            기존 데이터에 중복이 남아 있으면 생성이 실패한다 — 그 실패는
            **삼키지 않는다**(app_db 참조). 있다고 믿는데 없는 제약이 제일 나쁘다.
        """
        return []
